...
export default async function handler(req, res) {
  // 1. Pobieramy nazwę użytkownika z linku (np. ?username=MaxPowerPL)
  const { username } = req.query;

  // Zabezpieczenie: jeśli ktoś nie poda nicku
  if (!username) {
  ...
  try {
    ...
    // Używamy prostego HTML/SVG wewnątrz stringa
    const svg = `
      <svg width="400" height="200" viewBox="0 0 400 200" xmlns="http://www.w3.org/2000/svg">
        ...
        <text x="380" y="190" font-family="Arial" font-size="10" fill="#666" text-anchor="end">
    ...
    // Ważne: Mówimy przeglądarce, że to jest obrazek SVG, a nie tekst!
    res.setHeader('Content-Type', 'image/svg+xml');
    res.setHeader('Cache-Control', 's-maxage=60, stale-while-revalidate'); // Cache na 60 sekund
    res.status(200).send(svg);

  } catch (error) {
    // Obsługa błędu (np. nie ma takiego użytkownika)
    res.status(500).send('Błąd: Nie znaleziono użytkownika lub limit API wyczerpany.');
  }
}